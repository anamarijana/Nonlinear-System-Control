...
open('robusna_eFL_regulacija_nelin_sim.slx')
sim(['robusna_eFL_regulacija_nelin_sim.slx']);
...
%% 
%1.1
response3 = -S_out3+S_out3(1);
yend3 = response3(end);
idx3 = find(response3<=0.63*abs(yend3));
max_idx3 = idx3(end);
time_constant3 = t_out(max_idx3);
w0_estimated3 = 1./time_constant3;
disp('Propusni opseg regulacije, 1.1 Y:')
disp(w0_estimated3)
%% 
%1.2
response4 = -S_out4+S_out4(1);
yend = response4(end);
idx = find(response4<=0.63*abs(yend));
max_idx = idx(end);
time_constant = t_out(max_idx);
w0_estimated4 = 1./time_constant;
disp('Propusni opseg regulacije, 1.2 Y:')
disp(w0_estimated4)
